...
public abstract class AbstractPlayerAttachmentImmersive<I extends AbstractPlayerAttachmentInfo, S extends NetworkStorage> {
    ...
    private boolean nearbyItemGuideRenderCheck(I info) {
        ...
        // Use getWorldRenderPose for rendering to avoid jitter from tick desync
        Vec3 vrHitStart = inVR ? VRClientAPI.instance().getWorldRenderPose().getHead().getPos() : null;
        Vec3 vrLook = inVR ? VRClientAPI.instance().getWorldRenderPose().getHead().getDir() : null;
